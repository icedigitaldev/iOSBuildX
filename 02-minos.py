...
if not re.fullmatch(rb"[0-9]{2}\.[0-9]", TARGET):
    sys.exit("error: minimum OS version must be NN.N (15.0, 16.0, 17.0)")
...
if not hits:
    sys.exit("error: MinimumOSVersion constant not found in %s" % BINARY)
if len(hits) > 1:
    sys.exit("error: multiple MinimumOSVersion candidates in %s; binary left unchanged" % BINARY)

current = hits[0].group(1)
if current == TARGET:
    print("MinimumOSVersion is already %s" % TARGET.decode())
    sys.exit(0)
